module.exports = function(data) {
  const { name, imageNames, blocks, baseWidth, hotBlockFile } = data;
  return `
<!DOCTYPE html>
<html lang="en">
<head>
...
// ${hotBlockFile.name} - ${hotBlockFile.description}
<script src="${hotBlockFile.url}"></script>
<script>
    (function () {
        var blocks = [${blocks.map(item => `
            {
                name: '${item.name || ''}',
                spec: [ ${item.left}, ${item.top}, ${item.width}, ${item.height} ],
                onClick: function () {
                    // 来自于资源文件的处理函数
                    __HANDLE_BLOCK_CLICK('${item.action}', '${item.actionParam}');
                },
            }`)}
        ];
